...
import type { Score, Scorecard, State } from "../routes/route_check/data";
import {
  getFullCriticalIssue,
  getFullPolicyConflict,
} from "../routes/route_check/lists";
...
function summaryOfScheme(state: State, workbook: ExcelJS.Workbook) {
  ...
  sheet.getCell("C6").value = getDateForSpreadsheet(state.summary.dateDesignReview);
